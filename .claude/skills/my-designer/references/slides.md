...
PDF preserves rendered pages. PowerPoint keeps text, shapes, images and charts editable (including children of flex/grid frames) and rasterises only icons, 3D, creative embeds and component pages, listing rasterised layers in the slide notes; pass `--rasterize` for a picture-per-slide deck. Google Slides needs authorization and supports a narrower set of native text/shapes/HTTPS images; unsupported content fails explicitly. Open the actual exported deck, count pages, inspect the busiest slide, and report what remains editable. Saving JSON alone does not prove presentation fidelity.
