...
4. Resolve findings, save against the observed revision, and inspect again. The checker works on resolved layout geometry (flow children are judged where the frame places them) and uses bounded estimates — `text-overflow`, `text-spills-container` and `text-collision` sum glyph advances (Latin ≈ 0.52 em, CJK/kana/Hangul/fullwidth 1 em, combining marks 0) with kinsoku line breaking, so Japanese and NFD Vietnamese copy is measured as it renders; `text-contrast` compares against the topmost opaque frame or shape under the text, then the parent, then the page. It still does not certify fonts, gradients or image backdrops, transformed bounds, animated extremes, responsive behavior, or accessibility.
   Codes: `empty-page`, `zero-size`, `outside-page`, `missing-media`, `remote-media`, `empty-chart`, `empty-text`, `text-overflow`, `text-spills-container` (text extends past the card/frame beneath it), `text-collision` (two text layers overlap by more than 20% of the smaller), `text-contrast`, `export-duration` are warnings; `small-text`, `crowded-edge` (content within 2% of the page edge on web/slides/report pages) and `dense-page` are informational.
