...
4. Resolve findings, save against the observed revision, and inspect again. The checker uses bounded estimates — `text-overflow` sums glyph advances (Latin ≈ 0.52 em, CJK/kana/Hangul/fullwidth 1 em, combining marks 0) with kinsoku line breaking, so Japanese and NFD Vietnamese copy is measured as it renders, but it still does not certify fonts, composited contrast, transformed bounds, animated extremes, responsive behavior, or accessibility.
