...
Use text, shapes, imported images, and simple chart nodes for actual evidence. The chart node reads `data.labels` and `data.values`; review label fit and whether a basic bar chart suits the data. For exact analytical tables use a `table` node (`blocks get table`; `data.table.rows` of strings or `{text, colSpan, rowSpan, align, bold, fill}` cells, `headerRows`, `columnWidths`): it renders as a grid in PNG/PDF/SVG, exports as a native PowerPoint table and as `xlsx`/`csv`. A component `Table` only builds prototype rows from `props.items` with ordinal values. Retain accessible supporting text as the brief requires.
...
PDF is the primary page-faithful delivery choice; verify all pages in the actual file. JSON retains the editable document. PowerPoint keeps text, shapes, images and charts editable and rasterises only icons, creative embeds and component pages (see the slide notes for the list). HTML is a standalone designed document, not proof of document-editor semantics; there is no native DOCX export. `xlsx` (all pages with tables, one sheet each) and `csv` (first table on `--page`) hand the numbers to a spreadsheet. Import media before cloud rendering and keep an editable source alongside the final report when requested.
