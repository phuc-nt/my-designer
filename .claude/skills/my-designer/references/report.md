...
PDF is the primary page-faithful delivery choice; verify all pages in the actual file. JSON retains the editable document. PowerPoint keeps text, shapes, images and charts editable and rasterises only icons, creative embeds and component pages (see the slide notes for the list). HTML is a standalone designed document, not proof of document-editor semantics; there is no native DOCX export. Import media before cloud rendering and keep an editable source alongside the final report when requested.
