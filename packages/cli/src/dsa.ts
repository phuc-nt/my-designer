...
projects.command('export <id>').description('Export through the authenticated server renderer').requiredOption('--format <format>', 'json, html, svg, png, pdf, pptx, xlsx, csv, webm, mp4, react (ZIP), glb, gltf, motion (ZIP), png-sequence (ZIP), spritesheet (ZIP), scene-angles (ZIP), editable-scene (JSON)').option('-o, --output <file>', 'Output filename; required for binary formats').option('--out <file>', 'Alias for --output').option('--start <seconds>', 'Frame export start time').option('--end <seconds>', 'Frame export end time').option('--fps <number>', 'Frame export FPS').option('--review-samples <number>', 'Scene review samples (2–25)').option('--node <id>', 'Imported model for editable-scene export').option('--page <index>', 'Zero-based page for single-page exports', '0').option('--rasterize', 'PowerPoint: render every slide as one picture instead of editable text, shapes, images and charts').option('--revision <number>', 'Require the saved revision to match').action(async (id, options) => {
  if (!['json', 'html', 'svg', 'png', 'pdf', 'pptx', 'xlsx', 'csv', 'webm', 'mp4', 'react', 'glb', 'gltf', 'motion', 'png-sequence', 'spritesheet', 'scene-angles', 'editable-scene'].includes(options.format)) throw new CliError('unsupported_format', 'Formats: json, html, svg, png, pdf, pptx, xlsx, csv, webm, mp4, react, glb, gltf, motion, png-sequence, spritesheet, scene-angles, editable-scene. Use google-slides for Google Slides.');
  const destination = options.output ?? options.out;
  const binary = !['json', 'html', 'svg', 'gltf', 'csv'].includes(options.format);
