...
A design system can also be authored as a portable folder — `manifest.json` (id/name/description/system/source), `DESIGN.md` (agent-facing prose) and `tokens.css` (compiled custom properties) — and imported with `dsa design-systems import --folder <dir>`. `dsa design-systems export <id> --folder <dir>` reverses it. `dsa design-systems extract --from <dir> [--import]` derives such a folder from a codebase — CSS/SCSS custom properties, a `tailwind.config.*` read with regexes (never executed) and any existing `DESIGN.md` — mapping what it finds onto the canonical `--bg`/`--fg`/`--accent`/`--font-*`/`--radius`/`--space-N` tokens and reporting what is missing before the compile guard runs. The compiled versioned JSON remains the runtime authority; the folder is an ingest surface, not a second truth.

## Capability boundaries

The CLI's `projects export` requests real file bytes from `/api/projects/:id/export`; discover formats and fields through `/api/schema` and installed command help. Binary downloads require `--output` (or `--out`). Raster, motion and 3D formats require a configured Cloudflare/self-host browser renderer; missing configuration and unavailable encoders return errors. Optional `--revision` ensures the server exports the inspected revision, and `--page` selects a zero-based page where supported. React packages a runnable frontend prototype without a business backend. GLB/glTF preserve supported geometry, textures, skinning and sampled animation. Cloud rendering embeds owned assets; remote media must be imported first. Motion exports are capped at 60 seconds and MP4 requires encoder support. Browser and cloud motion exports share timeline audio cue timing and mixing. PowerPoint preserves editable text (font, alignment, spacing), shapes (fill, stroke, radius), images and native charts, places flex/grid children at their resolved positions, and rasterizes only icons, 3D, creative embeds and live artifacts per layer (listed in the slide notes); pages with React components render as one picture, and `--rasterize` (REST `rasterize: true`) renders every slide as a picture. `table` nodes (`data.table.rows` of cells with spans, header rows and per-cell fill/alignment) become native PowerPoint tables, and export on their own as `xlsx` (one sheet per page with tables, numeric text stored as numbers) or `csv` (the first table on `--page`) without a browser renderer.
