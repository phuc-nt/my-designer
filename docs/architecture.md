...
Live editing uses [three-way merge](../src/shared/document-merge.ts) through [collaboration routes](../server/collaboration.ts). The editor checks for remote revisions on a 1.2-second interval and merges independent field changes. Overlapping edits return explicit conflicts; the UI preserves its local state and pauses automatic writes. This is polling synchronization, not a WebSocket presence service. Undo snapshots are rebased against remote edits; incompatible snapshots are removed with a notice. Document writes retain the existing atomic owner/revision check and never alter brief approval. Review [comments](../src/shared/comments.ts) are stored on nodes and pages inside the document rather than in a table, so they share the revision, merge and history story; `GET /api/projects/:id/comments` walks the document and the `add-comment`/`resolve-comment` operations edit it. Document writes accept `?summary=1` and answer with the project summary plus a [structural diff](../src/shared/document-diff.ts) of touched pages and nodes; the CLI's `changes --follow` uses the same diff to summarise each human save.
...
JSON preserves the document. SVG preserves supported static structure; HTML can include the trusted 3D/timeline viewer. PNG renders the selected page; PDF/PowerPoint process all pages. PowerPoint retains editable text, shapes, images, charts and tables and rasterizes only unsupported layers (or every slide with `rasterize`). [Spreadsheet export](../src/shared/spreadsheet-export.ts) writes `xlsx` (OOXML through JSZip, one sheet per page with `table` nodes) and `csv` directly from the shared [table model](../src/shared/table.ts), so neither needs the browser renderer. Google Slides creates native text/shapes/HTTPS images and rejects unsupported complex nodes and private image references.
...
Rendered exports are cached in the asset bucket under `exports/<project>/<revision>/<options hash>` and indexed in `export_cache`; a repeated export of the same saved revision with the same options returns the stored bytes (`X-Export-Cache: hit`). Thumbnails, inspection renders, snapshot renders and the cheap `json`/`html`/`svg` formats bypass the cache. Saving a new revision evicts the older revision's entries on the next export, and project deletion removes the remaining bytes.
